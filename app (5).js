...
function renderMessage(message, sourceChannelId = state.activeChannelId) {
    ...
    if (embed) {
        ...
        header.append(sourceLabel, openBtn);
...
function createEmbed(url) {
    ...
    // Redgifs embeds - prefer a native video element loaded from the Redgifs API
    // (https://redgifs.com/watch/{id}). If the API fails, fall back to the iframe.
    try {
        ...
        if (host.includes("redgifs.com") || host.includes("redgif.com")) {
            ...
            if (id) {
                ...
                // We'll try to fetch the MP4 URL asynchronously and set it on the element.
                (async () => {
                    try {
                        const apiUrl = `https://api.redgifs.com/v2/gfycats/${encodeURIComponent(id)}`;
                        const resp = await fetch(apiUrl);
                        if (!resp.ok) throw new Error("Redgifs API error");
                        const data = await resp.json();

                        // Try a few known locations in the API response for MP4 URLs
                        const gfy = data?.gfyItem || data?.gfy || {};
                        const mp4 = gfy?.mp4Url || gfy?.mobileUrl || gfy?.urls?.hd || gfy?.urls?.sd || gfy?.gifUrl;

                        if (mp4) {
                            // set source and keep the native video element
                            video.src = mp4;
                        } else {
                            // fallback to iframe when we can't find a usable mp4
                            const iframe = document.createElement("iframe");
                            iframe.className = "embed";
